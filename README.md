...
If there is no activity in the repo for 60 days, the Github Action will be disabled and must be manually reanabled in the [Actions menu](https://github.com/jhublast/jhublast.github.io/actions/workflows/gh-pages.yml) and a new build must be initialized (e.g. by pushing a new commit).

## Developing
There are three types of primary content: research, people, events. After installing Hugo, a new section of each type can be generated by running `hugo new <research/people/events>/<filename>.md` from the root directory, which will populate a templated markdown file in `content/<research/people/events>/<filename>.md` that you can fill out. Without Hugo, you could just copy and paste an old markdown file from `content/` and edit it as needed. 
